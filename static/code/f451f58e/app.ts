...
new p5((p: p5) => {
  ...
  p.draw = () => {
    ...
    for (let i = 0; i < particles.length; i++) {
      ...
      if (particle.x + particle.radius < 0 || p.width <= particle.x - particle.radius || particle.y + particle.radius < 0 || p.height <= particle.y - particle.radius) {
  ...
  function addParticle(): void {
    ...
    const particle = init(p.windowWidth / 2, p.windowHeight / 2, vx, vy, radius, opacity);
