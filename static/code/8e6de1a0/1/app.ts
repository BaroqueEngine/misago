  ...
  function makeCorridor(partitions: Rectangle[], rooms: Rectangle[]): void {
    ...
    for (let i = 0; i < partitions.length; i++) {
      for (let j = i + 2; j < partitions.length; j++) {
        if (partitions[i].left - 1 === partitions[j].right + 1 || partitions[i].right + 1 === partitions[j].left - 1 || partitions[i].top - 1 === partitions[j].bottom + 1 || partitions[i].bottom + 1 === partitions[j].top - 1) {
          if (Math.random() < 0.5) {
